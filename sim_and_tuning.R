...
# To make the new plots clearer, let's filter for the best tuning parameters we identified:
# - A fine grid (num_N_hypothesized = 51)
# We will show both N_try_max_factor values using line type.
...
# Using facet_wrap for a more robust layout
plot_loci_effect <- ggplot(summary_stats_filtered,
                           aes(x = factor(n_loci), y = rmse, color = method, 
                               group = interaction(method, N_try_max_factor), linetype = factor(N_try_max_factor))) +
  geom_line(linewidth = 1.1) +
  geom_point(size = 3) +
  facet_wrap(~ Nc_Label + Fis_Label + Prop_Label, scales = "free_y", ncol = 4) +
  labs(
...
ggsave(file.path(output_dir, "plot_loci_effect_rmse.png"), plot_loci_effect, width = 16, height = 12)
...
plot_sampling_effect <- ggplot(summary_stats_filtered,
  ...
  geom_point(size = 3) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "blue") +
  facet_wrap(~ Nc_Label + Fis_Label + Loci_Label, scales = "free_y", ncol = 4) +
  scale_y_continuous(labels = scales::percent_format()) +
  labs(
...
ggsave(file.path(output_dir, "plot_sampling_effect_bias.png"), plot_sampling_effect, width = 16, height = 9)


message("All done. Check the '", output_dir, "' directory for results and plots.")


# **FIXED**: Group by all simulation and tuning parameters to get detailed summaries.
summary_stats <- results_list %>%
...
# PLOT 3: Effect of Number of Loci on Overall Performance (RMSE)
# **FIX**: Removed the explicit `group` aesthetic to let ggplot handle it.
plot_loci_effect <- ggplot(summary_stats_filtered,
                           aes(x = factor(n_loci), y = rmse, color = method, 
